...
class Position:
    ...
    def __post_init__(self):
        if self.tp_price is None:
            raise ValueError("tp_price is None. Construct positions via RiskManager.build_position().")
        if self.sl_price is None:
            raise ValueError("sl_price is None. Construct positions via RiskManager.build_position().")
    ...
    def rehydrate_exposure(self, market_id: str, trader_address: str, value: float, event_id: str = "") -> None:
        ...
        if mkt_exp_f > market_cap + _EPSILON:
            logger.warning(
                ...
                mkt_exp_f,
                market_cap,
                self.cfg.max_market_exposure_pct * 100,
                self.bankroll,
            )
        ...
        if trd_exp_f > trader_cap + _EPSILON:
            logger.warning(
                "Rehydrated allocation for trader %s = $%.2f exceeds current cap $%.2f (%.0f%% of $%.2f).",
                trader_address[:10],
                trd_exp_f,
                trader_cap,
                self.cfg.max_trader_allocation * 100,
                self.bankroll,
            )
...
def _assert_valid_price(price: float, name: str = "price") -> None:
    if not (0.0 <= price <= 1.0):
        raise InvalidPriceError(f"{name} must be in [0.0, 1.0] (Polymarket token range). Got: {price!r}")
